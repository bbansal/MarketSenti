...
/**
 * Gets a new article and based on a list of Objects (Company), tags them with 
 * a particluar company and stores it in a local database.  
 * @author abhishek
 *
 */
public class NewsArticleTagger {
	...
	public NewsArticleTagger(ArticleCompanyScorer scorer, StorageEngine storage){
		...
		storage.createStore("ArticleTagger", String.class, String.class);
	}
	
	/*
	 * This function assumes that raw articles from text extractor are all stores in a storeName with Key = String (TimeStamp) and Value = Article
	 */
	public void TagArticlesSinceTime(String sourceStoreName, long timestamp){
		...
		Iterator<Article> articleListIter = sourceIter.next().getValues(); 
		
		while(articleListIter.hasNext()){
			Article article = articleListIter.next();
